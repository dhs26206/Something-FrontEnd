...
const Repo=()=>{
    ...
    useEffect(() => {
        const fetchRepos = async () => {
          try {
            const resp = await fetch('https://admin.server.ddks.live/repos', {
              credentials: 'include'
            });
    ...
    const handleDeploy=(e)=>{
        window.location.href="/deploy?R="+e
    }
    return(
        <div className="h-screen w-screen">
            ...
            <div className="w-full h-[85%] overflow-y-scroll flex justify-center">
                <div className="h-[90%] w-[90%] flex gap-4 flex-wrap overflow-y-scroll">
                    {Repos.map((index,element)=>{
                        return(
                            <div id={index} onClick={()=>handleDeploy(element.name)} className="w-[19%] h-[19%] border-2 text-2xl font-white border-white shadow-lg hover:bg-green-900">
                                element.name
                            </div>
                        )
                    })}
